...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:tiger@Welcome123*/myflaskapp'
db = SQLAlchemy(app)
    ...
    # Close connection
       cur.close()
        ...
        if result>0 :
         ...
         if flag==True:
           cur = mysql.connection.cursor()
           cur.execute("INSERT INTO room_alloc(username,room_no,block_no,stat,warden) VALUES(%s, %s, %s,'unallocated',%s)", ([session['username']], room_no, block_no,warden))
...
def allot_room():
       ...
       cur = mysql.connection.cursor()
       result = cur.execute("SELECT room_alloc.username,room_alloc.room_no,room_alloc.block_no,users.GPA FROM users,room_alloc WHERE room_alloc.username =users.username and room_alloc.warden=%s ORDER BY GPA DESC",[session['username']])
...
def payment_mess():
    ...
    if request.method=='POST' and form.validate():
        ...
        # Execute query
        cur.execute("INSERT INTO mess_fee( _date,paid_by,transaction_no,amount) VALUES(%s, %s, %s, %s )", (start_date_str, paid_by, mobile_no, amt))
...
def ward_info():
       ...
       cur = mysql.connection.cursor()
       result = cur.execute("SELECT first_name,last_name,email,gender,GPA,contactno,mess_type FROM users WHERE  warden=%s",[session['username']] )
       students = cur.fetchall()
       if result > 0:
...
class create_roomForm(Form):
    room_no = SelectField('Room_number', choices=[(301,301),(302,302),(303,303),(304,304),(401,401),(402,402),(403,403),(404,404)])
    block_no = SelectField('Block_number', choices=[(3,3),(4,4)])
